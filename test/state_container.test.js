...
test("onUpdate listeners are fired when set is called", () => {
  ...
  expect(JSON.stringify(listener.args[0])).toBe(
    JSON.stringify([{ foo: "bar" }, { foo: "hi" }])
  );
...
test("onUpdate listeners are fired when setState is called", () => {
  ...
  expect(JSON.stringify(listener1.args[0])).toEqual(
    JSON.stringify([{ foo: "bar" }, { foo: "hi" }])
  );

  expect(listener2.args.length).toBe(2);
  expect(JSON.stringify(listener2.args[0])).toEqual(
    JSON.stringify([{ foo: "bar" }, { foo: "hi" }])
  );
  expect(JSON.stringify(listener2.args[1])).toEqual(
    JSON.stringify([{ foo: "hi" }, { foo: "unfoo" }])
  );
...
test("recorder getState works as expected", () => {
  ...
  expect(recorder.getState()).toEqual(
    { foo: { bar: 2 } },
    "replaces when set with complex value"
  );
  ...
  expect(recorder.getState()).toEqual(
    { foo: { bar: 2, baz: 1 } },
    "updates when set with simple value"
  );
});
...
test("lenses work", () => {
  ...
  expect(lens.get()).toEqual(
    { bar: 2, baz: 2 },
    "changing a nested lens updates its parent's value"
  );
  expect(container.getState()).toEqual(
    { foo: { bar: 2, baz: 2 } },
    "changing a nested lens updates container state"
  );

  lens.set({ bar: 3 });
  expect(sublens.get()).toBe(
    3,
    "changing parent lens to complex value updates nested lens value"
  );
  expect(lens.get()).toEqual(
    { bar: 3 },
    "changing parent lens to complex value updates itself"
  );
  expect(container.getState()).toEqual(
    { foo: { bar: 3 } },
    "changing parent lens to complex value updates container"
  );
  ...
  expect(
    lens.withValue((value, prop) => R.assoc(prop, value, { b: 2, a: 1 }), "a")
  ).toEqual({ a: 3, b: 2 });
  ...
  expect(recorder.getState()).toEqual(
    { foo: { bar: 2, baz: 2 } },
    "changing a nested lens updates recorder state"
  );
  ...
  expect(recorder.getState()).toEqual(
    { foo: { bar: 3 } },
    "changing parent lens to complex value updates recorder"
  );
